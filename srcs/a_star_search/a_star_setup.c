...
/*   Updated: 2025/03/28 12:18:56 by nlouis           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_closed_list	*create_closed_list(t_game *game)
{
	...
	closed_list = (t_closed_list *)malloc(sizeof(t_closed_list));
	closed_list->capacity = game->map.size.x * game->map.size.y;
	closed_list->nodes = (t_node **)malloc(sizeof(t_node *)
			* closed_list->capacity);
	if (!closed_list || !closed_list->nodes)
		perror_free_and_exit("Error\nMalloc failed for in create_closed_list",
			game);
	closed_list->size = 0;
	return (closed_list);
}
...
static int	**create_open_list(t_game *game)
{
	...
	open_list = (int **)malloc(sizeof(int *) * game->map.size.y);
	if (open_list == NULL)
		perror_free_and_exit("Error\nMalloc failed for open_list\n", game);
	y = 0;
	while (y < game->map.size.y)
	{
		open_list[y] = ((int *)ft_calloc(game->map.size.x, sizeof(int)));
		if (!open_list[y])
		{
			while (y-- > 0)
				free(open_list[y]);
			free(open_list);
			perror_free_and_exit("Error\nCalloc failed for open_list\n", game);
		}
		y++;
	}
	return (open_list);
...
static t_node	*create_start_node(t_game *game, t_astar *astar, t_point start)
{
	...
	if (!start_node)
		perror_free_and_exit("Error\nMalloc failed for start_node\n", game);
	start_node->next = NULL;
	start_node->pos = start;
	start_node->g_cost = 0;
	start_node->h_cost = ft_manhattan_dist_point(start_node->pos, astar->goal);
...
void	setup_astar_struct(t_game *game, t_astar *astar, t_point start,
	t_point goal)
{
	if (astar->closed_list != NULL)
		reset_astar_struct(astar, game);
	astar->goal = goal;
